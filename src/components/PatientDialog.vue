<template>
  <div class="text-center">
    <v-dialog
        v-model="dialog"
        width="auto"
    >
      <template v-slot:activator="{ props }">
        <div>
          <v-btn v-bind="props" style="margin-right: 1vw;" density="compact" :icon="actionIcon"></v-btn>
        </div>
      </template>

      <v-card v-if="isEdit">
        <v-card-title>
          Редактирование данных пациента
        </v-card-title>
        <v-card-item>
          <v-text-field label="ФИО" v-model="localData.fullName"></v-text-field>
          <v-select
              label="Гендер"
              v-model="localData.gender"
              :items="['Муж.', 'Жен.', 'Неопределеный']"
          ></v-select>
          <v-text-field label="Телефон" v-model="localData.telephone"></v-text-field>
          <v-text-field label="Снилс" v-model="localData.snils"></v-text-field>
        </v-card-item>
        <v-card-actions>
          <v-btn @click="changeData">Отправить</v-btn>
        </v-card-actions>
      </v-card>
      <v-card v-else>
        <v-card-text>
          Вы действительно хотите удалить пациента?
        </v-card-text>
      </v-card>
    </v-dialog>
  </div>
</template>

<script>
export default {
  name: "PatientDialog",
  props: {
    actionType: String,
    patient: {
      id: Number,
      fullName: String,
      birthDay: String,
      gender: String,
      telephone: String,
      snils: String
    }
  },
  mounted() {
    this.localData = {
      ...this.patient
    }
  },
  data() {
    return {
      dialog: false,
      localData: {
        id: '',
        fullName: '',
        birthDay: '',
        gender: '',
        telephone: '',
        snils: ''
      }
    }
  },
  methods: {
    changeData() {
      this.$emit('changePatientData', this.localData)
    }
  },
  computed: {
    actionIcon() {
      if (this.actionType === 'DELETE') {
        return 'mdi-delete'
      }

      if (this.actionType === 'EDIT') {
        return 'mdi-pencil'
      }
    },
    isEdit() {
      return this.actionType === 'EDIT'
    },
  },
  watch: {
    patient(value) {
      this.localData = {
        ...value
      }
    }
  }
}
</script>

<style scoped>

</style>